...
class Timetable extends AppModel {
	 ...
	 * 時間割エンティティを取得
	 *
	 * 時間割テーブルからidに紐づくエンティティを取得します。
	 *
	 * @param int $id 時間割ID
	 * @return array 時間割エンティティ
	 */
	...
	private function dumpSql($option){
		...
		$option['table']=$dbo->fullTableName($this->Timetable);
		$option['alias']='Timetable';
		
		$query = $dbo->buildStatement($option,$this->Timetable);
	...
	private function createKjConditions($kjs){
		...
		if(!empty($kjs['kj_main'])){
			$cnds[]="CONCAT( IFNULL(Timetable.timetable_name, '') ,IFNULL(Timetable.note, '')) LIKE '%{$kjs['kj_main']}%'";
		}
		
		// CBBXS-1003
		if(!empty($kjs['kj_id']) || $kjs['kj_id'] ==='0' || $kjs['kj_id'] ===0){
			$cnds[]="Timetable.id = {$kjs['kj_id']}";
		}
		if(!empty($kjs['kj_timetable_name'])){
			$cnds[]="Timetable.timetable_name LIKE '%{$kjs['kj_timetable_name']}%'";
		}
		if(!empty($kjs['kj_rental_item_id']) || $kjs['kj_rental_item_id'] ==='0' || $kjs['kj_rental_item_id'] ===0){
			$cnds[]="Timetable.rental_item_id = {$kjs['kj_rental_item_id']}";
		}
		if(!empty($kjs['kj_note'])){
			$cnds[]="Timetable.note LIKE '%{$kjs['kj_note']}%'";
		}
		if(!empty($kjs['kj_sort_no']) || $kjs['kj_sort_no'] ==='0' || $kjs['kj_sort_no'] ===0){
			$cnds[]="Timetable.sort_no = {$kjs['kj_sort_no']}";
		}
		$kj_delete_flg = $kjs['kj_delete_flg'];
		if(!empty($kjs['kj_delete_flg']) || $kjs['kj_delete_flg'] ==='0' || $kjs['kj_delete_flg'] ===0){
			if($kjs['kj_delete_flg'] != -1){
			   $cnds[]="Timetable.delete_flg = {$kjs['kj_delete_flg']}";
			}
		}
		if(!empty($kjs['kj_update_user'])){
			$cnds[]="Timetable.update_user LIKE '%{$kjs['kj_update_user']}%'";
		}
		if(!empty($kjs['kj_ip_addr'])){
			$cnds[]="Timetable.ip_addr LIKE '%{$kjs['kj_ip_addr']}%'";
		}
		if(!empty($kjs['kj_created'])){
			$kj_created=$kjs['kj_created'].' 00:00:00';
			$cnds[]="Timetable.created >= '{$kj_created}'";
		}
		if(!empty($kjs['kj_modified'])){
			$kj_modified=$kjs['kj_modified'].' 00:00:00';
			$cnds[]="Timetable.modified >= '{$kj_modified}'";
		}
	 ...
	 * エンティティをDB保存
	 *
	 * 時間割エンティティを時間割テーブルに保存します。
	 *
	 * @param array $ent 時間割エンティティ
	 * @param array $option オプション
	 *  - form_type フォーム種別  new_inp:新規入力 , copy:複製 , edit:編集
	 *  - ni_tr_place 新規入力追加場所フラグ 0:末尾 , 1:先頭
	 * @return array 時間割エンティティ（saveメソッドのレスポンス）
	 */
	public function saveEntity($ent,$option=array()){
		...
		$ent = $this->find('first',
				array(
						'conditions' => "id={$ent['Timetable']['id']}"
				));
	...
	// CBBXS-1021
	/**
	 * レンタルアイテムリストをDBから取得する
	 */
	public function getRentalItemIdList(){
		if(empty($this->RentalItem)){
			App::uses('RentalItem','Model');
			$this->RentalItem=ClassRegistry::init('RentalItem');
		}
		$fields=array('id','rental_item_name');//SELECT情報
		...
		$data=$this->RentalItem->find('all',$option); // DBから取得
		
		// 構造変換
		if(!empty($data)){
			$data = Hash::combine($data, '{n}.RentalItem.id','{n}.RentalItem.rental_item_name');
		}
