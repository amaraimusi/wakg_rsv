<?php
$this->CrudBase->init(array('model_name'=>'Timetable'));

// CSSファイルのインクルード
$cssList = $this->CrudBase->getCssList();
$cssList[] = 'Timetable/index'; // 当画面専用CSS
$this->assign('css', $this->Html->css($cssList));

// JSファイルのインクルード
$jsList = $this->CrudBase->getJsList();
$jsList[] = 'Timetable/index'; // 当画面専用JavaScript
...
<div class="cb_func_line">
	...
	<div class="cb_kj_main">
		<!-- 検索条件入力フォーム -->
		<?php echo $this->Form->create('Timetable', array('url' => true , 'class' => 'form_kjs')); ?>
		<?php $this->CrudBase->inputKjMain($kjs,'kj_main','',null,'時間割名、備考を検索する');?>
		...
		$this->CrudBase->inputKjText($kjs,'kj_timetable_name','割当名');
		$this->CrudBase->inputKjSelect($kjs,'kj_rental_item_id','レンタルアイテム',$rentalItemIdList); 
...
<?php 
	...
	$csv_dl_url = $this->html->webroot . 'timetable/csv_download';
...
<!-- 一覧テーブル -->
<table id="timetable_tbl" border="1"  class="table table-striped table-bordered table-condensed" style="display:none;margin-bottom:0px">
...
foreach($data as $i=>$ent){
	...
	$this->CrudBase->tdStr($ent,'timetable_name');
	$this->CrudBase->tdList($ent,'rental_item_id',$rentalItemIdList);
...
<table id="crud_base_forms">
	...
	<tr id="ajax_crud_new_inp_form" class="crud_base_form" style="display:none;padding-bottom:60px"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp' >割当名: </div>
			<div class='cbf_input'>
				<input type="text" name="timetable_name" class="valid " value=""  maxlength="255" title="255文字以内で入力してください" />
				<label class="text-danger" for="timetable_name"></label>
			</div>
		</div>

		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >レンタルアイテム: </div>
			<div class='cbf_input'>
				<?php $this->CrudBase->selectX('rental_item_id',null,$rentalItemIdList,null);?>
				<label class="text-danger" for="rental_item_id"></label>
			</div>
		</div>
...
<div id="ajax_crud_delete_form" class="panel panel-danger" style="display:none">
	...
	<table><tbody>
		...
		<tr><td>時間割名: </td><td>
			<span class="timetable_name"></span>
		</td></tr>
...
<div style="display:none">
	...
	<!-- CBBXS-1022 -->
	<input id="rental_item_id_json" type="hidden" value='<?php echo $rental_item_id_json; ?>' />
