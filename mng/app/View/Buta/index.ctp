<?php
$this->CrudBase->init(array('model_name'=>'Buta'));

// CSSファイルのインクルード
$cssList = $this->CrudBase->getCssList();
$cssList[] = 'Buta/index'; // 当画面専用CSS
$this->assign('css', $this->Html->css($cssList));

// JSファイルのインクルード
$jsList = $this->CrudBase->getJsList();
$jsList[] = 'Buta/index'; // 当画面専用JavaScript
...
<div class="cb_func_line">
	...
	<div class="cb_kj_main">
		<!-- 検索条件入力フォーム -->
		<?php echo $this->Form->create('Buta', array('url' => true )); ?>
		<?php $this->CrudBase->inputKjMain($kjs,'kj_main','',null,'ブタ名前、ブタ日、備考を検索する');?>
...
<?php 
	...
	$csv_dl_url = $this->html->webroot . 'buta/csv_download';
...
<!-- 一覧テーブル -->
<table id="buta_tbl" border="1"  class="table table-striped table-bordered table-condensed">
...
foreach($data as $i=>$ent){
	...
	$this->CrudBase->tdPlain($ent,'neko_val');
	$this->CrudBase->tdStr($ent,'neko_name');
	$this->CrudBase->tdPlain($ent,'neko_date');
	$this->CrudBase->tdPlain($ent,'neko_group');
