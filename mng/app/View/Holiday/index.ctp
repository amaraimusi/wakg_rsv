<?php
$this->CrudBase->init(array('model_name'=>'Holiday'));

// CSSファイルのインクルード
$cssList = $this->CrudBase->getCssList();
$cssList[] = 'Holiday/index'; // 当画面専用CSS
$this->assign('css', $this->Html->css($cssList));

// JSファイルのインクルード
$jsList = $this->CrudBase->getJsList();
$jsList[] = 'Holiday/index'; // 当画面専用JavaScript
...
<div class="cb_func_line">
	...
	<div class="cb_kj_main">
		<!-- 検索条件入力フォーム -->
		<?php echo $this->Form->create('Holiday', array('url' => true , 'class' => 'form_kjs')); ?>
		<?php $this->CrudBase->inputKjMain($kjs,'kj_main','',null,'休日名、休日日付、備考を検索する');?>
		...
		$this->CrudBase->inputKjText($kjs,'kj_holiday_name','休日名');
		$this->CrudBase->inputKjText($kjs,'kj_holiday_date','休日日付');
		$this->CrudBase->inputKjSelect($kjs,'kj_holiday_type','休日タイプ',$holidayTypeList); 
...
<?php 
	...
	$csv_dl_url = $this->html->webroot . 'holiday/csv_download';
...
<!-- 一覧テーブル -->
<table id="holiday_tbl" border="1"  class="table table-striped table-bordered table-condensed" style="display:none;margin-bottom:0px">
...
foreach($data as $i=>$ent){
	...
	$this->CrudBase->tdStr($ent,'holiday_name');
	$this->CrudBase->tdPlain($ent,'holiday_date');
	$this->CrudBase->tdList($ent,'holiday_type',$holidayTypeList);
...
<table id="crud_base_forms">
	...
	<tr id="ajax_crud_new_inp_form" class="crud_base_form" style="display:none;padding-bottom:60px"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp' >休日名: </div>
			<div class='cbf_input'>
				<input type="text" name="holiday_name" class="valid " value=""  maxlength="255" title="255文字以内で入力してください" />
				<label class="text-danger" for="holiday_name"></label>
			</div>
		</div>

		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >休日日付: </div>
			<div class='cbf_input'>
				<input type="text" name="holiday_date" class="valid datepicker" value=""  pattern="([0-9]{4})(/|-)([0-9]{1,2})(/|-)([0-9]{1,2})" title="日付形式（Y-m-d）で入力してください(例：2012-12-12)" />
				<label class="text-danger" for="holiday_date"></label>
			</div>
		</div>
		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >休日タイプ: </div>
			<div class='cbf_input'>
				<?php $this->CrudBase->selectX('holiday_type',null,$holidayTypeList,null);?>
				<label class="text-danger" for="holiday_type"></label>
			</div>
		</div>
...
<div style="display:none">
	...
	<!-- CBBXS-1022 -->
	<input id="holiday_type_json" type="hidden" value='<?php echo $holiday_type_json; ?>' />
