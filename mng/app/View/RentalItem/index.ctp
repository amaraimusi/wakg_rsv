<?php
$this->CrudBase->init(array('model_name'=>'RentalItem'));

// CSSファイルのインクルード
$cssList = $this->CrudBase->getCssList();
$cssList[] = 'RentalItem/index'; // 当画面専用CSS
$this->assign('css', $this->Html->css($cssList));

// JSファイルのインクルード
$jsList = $this->CrudBase->getJsList();
$jsList[] = 'RentalItem/index'; // 当画面専用JavaScript
...
<div class="cb_func_line">
	...
	<div class="cb_kj_main">
		<!-- 検索条件入力フォーム -->
		<?php echo $this->Form->create('RentalItem', array('url' => true , 'class' => 'form_kjs')); ?>
		<?php $this->CrudBase->inputKjMain($kjs,'kj_main','',null,'レンタルアイテム名前、レンタルアイテム日、備考を検索する');?>
		...
		$this->CrudBase->inputKjText($kjs,'kj_rental_item_name','レンタルアイテム名');
...
<?php 
	...
	$csv_dl_url = $this->html->webroot . 'rental_item/csv_download';
...
<!-- 一覧テーブル -->
<table id="rental_item_tbl" border="1"  class="table table-striped table-bordered table-condensed" style="display:none;margin-bottom:0px">
...
foreach($data as $i=>$ent){
	...
	$this->CrudBase->tdStr($ent,'rental_item_name');
...
<table id="crud_base_forms">
	...
	<tr id="ajax_crud_new_inp_form" class="crud_base_form" style="display:none;padding-bottom:60px"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp' >レンタルアイテム名: </div>
			<div class='cbf_input'>
				<input type="text" name="rental_item_name" class="valid " value=""  maxlength="255" title="255文字以内で入力してください" />
				<label class="text-danger" for="rental_item_name"></label>
			</div>
		</div>
...
<div id="ajax_crud_delete_form" class="panel panel-danger" style="display:none">
	...
	<table><tbody>
		...
		<tr><td>レンタルアイテム名: </td><td>
			<span class="rental_item_name"></span>
		</td></tr>
