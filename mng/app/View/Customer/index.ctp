<?php
$this->CrudBase->init(array('model_name'=>'Customer'));

// CSSファイルのインクルード
$cssList = $this->CrudBase->getCssList();
$cssList[] = 'Customer/index'; // 当画面専用CSS
$this->assign('css', $this->Html->css($cssList));

// JSファイルのインクルード
$jsList = $this->CrudBase->getJsList();
$jsList[] = 'Customer/index'; // 当画面専用JavaScript
...
<div class="cb_func_line">
	...
	<div class="cb_kj_main">
		<!-- 検索条件入力フォーム -->
		<?php echo $this->Form->create('Customer', array('url' => true , 'class' => 'form_kjs')); ?>
		<?php $this->CrudBase->inputKjMain($kjs,'kj_main','',null,'お客様氏名, ヨミガナ, メールアドレス, 郵便番号, 住所, 電話番号, 備考を検索する');?>
		...
		$this->CrudBase->inputKjText($kjs,'kj_customer_name','お客様氏名');
		$this->CrudBase->inputKjText($kjs,'kj_kana','ヨミガナ');
		$this->CrudBase->inputKjText($kjs,'kj_mail_addr','メールアドレス');
		$this->CrudBase->inputKjText($kjs,'kj_postal_code','郵便番号');
		$this->CrudBase->inputKjText($kjs,'kj_address','住所');
...
<?php 
	...
	$csv_dl_url = $this->html->webroot . 'customer/csv_download';
...
<!-- 一覧テーブル -->
<table id="customer_tbl" border="1"  class="tbl2" style="display:none;margin-bottom:0px">
...
foreach($data as $i=>$ent){
	...
	$this->CrudBase->tdStr($ent,'customer_name');
	$this->CrudBase->tdStr($ent,'kana');
	$this->CrudBase->tdStr($ent,'mail_addr');
	$this->CrudBase->tdStr($ent,'postal_code');
	$this->CrudBase->tdStr($ent,'address');
...
<table id="crud_base_forms">
	...
	<tr id="ajax_crud_new_inp_form" class="crud_base_form" style="display:none;padding-bottom:60px"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp' >お客様氏名: </div>
			<div class='cbf_input'>
				<input type="text" name="customer_name" class="valid " value=""  maxlength="255" title="255文字以内で入力してください" />
				<label class="text-danger" for="customer_name"></label>
			</div>
		</div>

		<div class="cbf_inp_wrap">
			<div class='cbf_inp' >ヨミガナ: </div>
			<div class='cbf_input'>
				<input type="text" name="kana" class="valid " value=""  maxlength="256" title="256文字以内で入力してください" />
				<label class="text-danger" for="kana"></label>
			</div>
		</div>
		...
		<div class="cbf_inp_wrap">
			...
			<div class='cbf_input'>
				<input type="text" name="mail_addr" class="valid " value=""  maxlength="256" title="256文字以内で入力してください" />
				<label class="text-danger" for="mail_addr"></label>
			</div>
		</div>

		<div class="cbf_inp_wrap">
			<div class='cbf_inp' >郵便番号: </div>
			<div class='cbf_input'>
				<input type="text" name="postal_code" class="valid " value=""  maxlength="16" title="16文字以内で入力してください" />
				<label class="text-danger" for="postal_code"></label>
			</div>
		</div>
		...
		<div class="cbf_inp_wrap">
			...
			<div class='cbf_input'>
				<input type="text" name="address" class="valid " value=""  maxlength="256" title="256文字以内で入力してください" />
				<label class="text-danger" for="address"></label>
			</div>
		</div>
		...
		<div class="cbf_inp_wrap">
			...
			<div class='cbf_input'>
				<input type="text" name="tel" class="valid " value=""  maxlength="32" title="32文字以内で入力してください" />
