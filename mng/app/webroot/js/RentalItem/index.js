...
$(function() {
	...
	$('#rental_item_tbl').show();// 高速表示のためテーブルは最後に表示する
...
function init(){
	...
	crudBase = new CrudBase({
			'src_code':'rental_item', // 画面コード（スネーク記法)
			'kjs':kjs,
			'ni_tr_place':1,
		});
	...
	pwms = new ProcessWithMultiSelection({
		'tbl_slt':'#rental_item_tbl',
		'ajax_url':'rental_item/ajax_pwms',
			});
	...
	// リアクト機能の初期化
	crudBase.reactInit('rental_item_tbl, hyo2');
}
...
function calendarViewKShow(){
	// カレンダービューを生成 
	crudBase.calendarViewCreate('rental_item_date');
}
