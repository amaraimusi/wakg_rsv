...
$(function() {
	...
	$('#timetable_tbl').show();// 高速表示のためテーブルは最後に表示する
...
function init(){
	...
	crudBase = new CrudBase({
			'src_code':'timetable', // 画面コード（スネーク記法)
			'kjs':kjs,
			'ni_tr_place':1,
		});
	...
	// CBBXS-1023
	// レンタルアイテムリストJSON
	var rental_item_id_json = jQuery('#rental_item_id_json').val();
	var rentalItemIdList = JSON.parse(rental_item_id_json);
	disFilData['rental_item_id'] ={'fil_type':'select','option':{'list':rentalItemIdList}};
	...
	pwms = new ProcessWithMultiSelection({
		'tbl_slt':'#timetable_tbl',
		'ajax_url':'timetable/ajax_pwms',
			});
	...
	// リアクト機能の初期化
	crudBase.reactInit('timetable_tbl, hyo2');
}
...
function calendarViewKShow(){
	// カレンダービューを生成 
	crudBase.calendarViewCreate('timetable_date');
}
