...
$(() => {
	...
	var select_option_html = createSelectOptionHtml(langData);
	$('#lang_list').html(select_option_html);
});
...
function translateExe(){
	...
	var lang = $('#lang_list option:selected').val();// 言語コードを取得
	var text1 = $('#contents1').html();// テキストを取得
	...
	$.ajax({
		type: "POST",
		url: "/cake_demo/wp_post/google_translation_ajax",
		data: "key1="+json_str,
		cache: false,
		dataType: "text",
	})
 ...
 */
function createSelectOptionHtml(langData){
	var html = '';
...
function getLangData(){
	...
	var langData = [
		...
		{"lang_nation":"朝鮮語族","wamei":"朝鮮語","lang_name":"한국어 (韓國語),조선어 (朝鮮語)","iso639-1":"ko","iso639-2T":"kor","iso639-2B":"kor","iso639-3":"kor","iso639-6":"","note":"","google_api":true},
		...
		{"lang_nation":"インド・ヨーロッパ語族","wamei":"ルーマニア語、モルドバ語[1]","lang_name":"limba română,limba moldovenească","iso639-1":"ro","iso639-2T":"ron","iso639-2B":"rum","iso639-3":"ron","iso639-6":"","note":"","google_api":true},
