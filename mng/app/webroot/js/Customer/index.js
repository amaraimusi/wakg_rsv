...
$(function() {
	...
	$('#customer_tbl').show();// 高速表示のためテーブルは最後に表示する
...
function init(){
	...
	crudBase = new CrudBase({
			'src_code':'customer', // 画面コード（スネーク記法)
			'kjs':kjs,
			'ni_tr_place':1,
		});
	...
	pwms = new ProcessWithMultiSelection({
		'tbl_slt':'#customer_tbl',
		'ajax_url':'customer/ajax_pwms',
			});
	...
	// リアクト機能の初期化
	crudBase.reactInit('customer_tbl, hyo2');
}
...
function calendarViewKShow(){
	// カレンダービューを生成 
	crudBase.calendarViewCreate('customer_date');
}
