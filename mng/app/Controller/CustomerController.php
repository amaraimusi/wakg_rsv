...
class CustomerController extends CrudBaseController {
	...
	public $uses = array('Customer','CrudBase');
	
	/// オリジナルヘルパーの登録[CakePHPの機能]
	public $helpers = array('CrudBase');

	/// デフォルトの並び替え対象フィールド
	public $defSortFeild='Customer.sort_no';
	...
	//public $viaDpFnMap = array('img_fn'=>'customer_group');
	...
	public function index() {
		...
		// CrudBase共通処理（前）
		$crudBaseData = $this->indexBefore('Customer');//indexアクションの共通先処理(CrudBaseController)
		
		//一覧データを取得
		$data = $this->Customer->findData($crudBaseData);
...
// 		}
		...
		// エンティティをDB保存
		$this->Customer->begin();
		$ent = $this->Customer->saveEntity($ent,$regParam);
		$this->Customer->commit();//コミット
	...
	public function ajax_delete(){
		...
		$this->Customer->begin();
		if($eliminate_flg == 0){
			$ent = $this->Customer->saveEntity($ent,$regParam); // 更新
		}else{
			$this->Customer->eliminateFiles($ent['id'], 'img_fn', $ent, $this->dp_tmpl, $this->viaDpFnMap); // ファイル抹消（他のレコードが保持しているファイルは抹消対象外）
			$this->Customer->delete($ent['id']); // 削除
		}
		$this->Customer->commit();//コミット
	...
	*/
	public function auto_save(){
		...
		// データ保存
		$this->Customer->begin();
		$this->Customer->saveAll($data); // まとめて保存。内部でSQLサニタイズされる。
	...
	public function csv_fu(){
		...
		$this->csv_fu_base($this->Customer,array('id','customer_val','customer_name','customer_date','customer_group','customer_dt','customer_flg','img_fn','note','sort_no'));
	...
	private function getDataForDownload(){
		...
		//セッションから検索条件情報を取得
		$kjs=$this->Session->read('customer_kjs');
		
		// セッションからページネーション情報を取得
		$pages = $this->Session->read('customer_pages');
	...
	private function initCrudBase(){
		...
		$this->kensakuJoken=array(
			...
			array('name'=>'kj_customer_name','def'=>null),
			array('name'=>'kj_kana','def'=>null),
			array('name'=>'kj_mail_addr','def'=>null),
			array('name'=>'kj_postal_code','def'=>null),
			array('name'=>'kj_address','def'=>null),
		...
		$this->kjs_validate=array(
				...
				'kj_customer_name'=> array(
						'maxLength'=>array(
								'rule' => array('maxLength', 255),
								'message' => 'お客様氏名は255文字以内で入力してください',
								'allowEmpty' => true
						),
				),
				'kj_kana'=> array(
				...
				'kj_postal_code'=> array(
						'maxLength'=>array(
								'rule' => array('maxLength', 255),
								'message' => '郵便番号は16文字以内で入力してください',
								'allowEmpty' => true
						),
				),
				'kj_address'=> array(
		...
		$this->field_data = array('def'=>array(
			...
			'id'=>array(
					'name'=>'ID',//HTMLテーブルの列名
					'row_order'=>'Customer.id',//SQLでの並び替えコード
					'clm_show'=>1,//デフォルト列表示 0:非表示 1:表示
			),
			'customer_name'=>array(
					'name'=>'お客様氏名',
					'row_order'=>'Customer.customer_name',
					'clm_show'=>1,
			),
			'kana'=>array(
					'name'=>'ヨミガナ',
					'row_order'=>'Customer.kana',
					'clm_show'=>1,
			),
			'mail_addr'=>array(
					'name'=>'メールアドレス',
					'row_order'=>'Customer.mail_addr',
					'clm_show'=>1,
			),
			'postal_code'=>array(
					'name'=>'郵便番号',
					'row_order'=>'Customer.postal_code',
					'clm_show'=>1,
			),
			'address'=>array(
					'name'=>'住所',
					'row_order'=>'Customer.address',
					'clm_show'=>1,
			),
			'tel'=>array(
					'name'=>'電話番号',
					'row_order'=>'Customer.tel',
					'clm_show'=>1,
			),
			'note'=>array(
					'name'=>'備考',
					'row_order'=>'Customer.note',
					'clm_show'=>0,
			),
			'sort_no'=>array(
					'name'=>'順番',
					'row_order'=>'Customer.sort_no',
					'clm_show'=>0,
			),
			'delete_flg'=>array(
					'name'=>'無効フラグ',
					'row_order'=>'Customer.delete_flg',
					'clm_show'=>0,
			),
			'update_user'=>array(
					'name'=>'更新者',
					'row_order'=>'Customer.update_user',
					'clm_show'=>0,
			),
			'ip_addr'=>array(
					'name'=>'IPアドレス',
					'row_order'=>'Customer.ip_addr',
					'clm_show'=>0,
			),
			'created'=>array(
					'name'=>'生成日時',
					'row_order'=>'Customer.created',
					'clm_show'=>0,
			),
			'modified'=>array(
					'name'=>'更新日',
					'row_order'=>'Customer.modified',
					'clm_show'=>0,
			),
