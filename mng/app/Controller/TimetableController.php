 ...
 * 
 * @note
 * 時間割画面では時間割一覧を検索閲覧、編集など多くのことができます。
 * 
 * @date 2018-12-28
 * @version 0.8.0
 *
...
class TimetableController extends CrudBaseController {
	...
	public $uses = array('Timetable','CrudBase');
	
	/// オリジナルヘルパーの登録[CakePHPの機能]
	public $helpers = array('CrudBase');

	/// デフォルトの並び替え対象フィールド
	public $defSortFeild='Timetable.sort_no';
	...
	//public $viaDpFnMap = array('img_fn'=>'timetable_group');
	...
	public function index() {
		...
		// CrudBase共通処理（前）
		$crudBaseData = $this->indexBefore('Timetable');//indexアクションの共通先処理(CrudBaseController)
		
		//一覧データを取得
		$data = $this->Timetable->findData($crudBaseData);
		...
		// レンタルアイテムリスト
		$rentalItemIdList = $this->Timetable->getRentalItemIdList();
		$rental_item_id_json = json_encode($rentalItemIdList,JSON_HEX_TAG | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_HEX_APOS);
		$this->set(array('rentalItemIdList' => $rentalItemIdList,'rental_item_id_json' => $rental_item_id_json));

		// CBBXE
		
		$this->set($crudBaseData);
		$this->set(array(
			'title_for_layout'=>'時間割',
			'data'=> $data,
		));
...
// 		}
		...
		// エンティティをDB保存
		$this->Timetable->begin();
		$ent = $this->Timetable->saveEntity($ent,$regParam);
		$this->Timetable->commit();//コミット
	...
	public function ajax_delete(){
		...
		$this->Timetable->begin();
		if($eliminate_flg == 0){
			$ent = $this->Timetable->saveEntity($ent,$regParam); // 更新
		}else{
			$this->Timetable->eliminateFiles($ent['id'], 'img_fn', $ent, $this->dp_tmpl, $this->viaDpFnMap); // ファイル抹消（他のレコードが保持しているファイルは抹消対象外）
			$this->Timetable->delete($ent['id']); // 削除
		}
		$this->Timetable->commit();//コミット
	...
	*/
	public function auto_save(){
		...
		// データ保存
		$this->Timetable->begin();
		$this->Timetable->saveAll($data); // まとめて保存。内部でSQLサニタイズされる。
	...
	public function csv_fu(){
		...
		$this->csv_fu_base($this->Timetable,array('id','timetable_val','timetable_name','timetable_date','timetable_group','timetable_dt','timetable_flg','img_fn','note','sort_no'));
	...
	private function getDataForDownload(){
		...
		//セッションから検索条件情報を取得
		$kjs=$this->Session->read('timetable_kjs');
		
		// セッションからページネーション情報を取得
		$pages = $this->Session->read('timetable_pages');
	...
	private function initCrudBase(){
		...
		$this->kensakuJoken=array(
			...
			array('name'=>'kj_timetable_name','def'=>null),
			array('name'=>'kj_rental_item_id','def'=>null),
		...
		$this->kjs_validate=array(
				...
				'kj_timetable_name'=> array(
						'maxLength'=>array(
								'rule' => array('maxLength', 255),
								'message' => '割当名は255文字以内で入力してください',
								'allowEmpty' => true
						),
				),
				'kj_rental_item_id' => array(
						'custom'=>array(
								'rule' => array( 'custom', '/^[-]?[0-9]+$/' ),
								'message' => 'レンタルアイテムは整数を入力してください。',
								'allowEmpty' => true
						),
				),
				'kj_note'=> array(
		...
		$this->field_data = array('def'=>array(
			...
			'id'=>array(
					'name'=>'ID',//HTMLテーブルの列名
					'row_order'=>'Timetable.id',//SQLでの並び替えコード
					'clm_show'=>1,//デフォルト列表示 0:非表示 1:表示
			),
			'timetable_name'=>array(
					'name'=>'割当名',
					'row_order'=>'Timetable.timetable_name',
					'clm_show'=>1,
			),
			'rental_item_id'=>array(
					'name'=>'レンタルアイテム',
					'row_order'=>'Timetable.rental_item_id',
					'clm_show'=>1,
			),
			'note'=>array(
					'name'=>'備考',
					'row_order'=>'Timetable.note',
					'clm_show'=>1,
			),
			'sort_no'=>array(
					'name'=>'順番',
					'row_order'=>'Timetable.sort_no',
					'clm_show'=>0,
			),
			'delete_flg'=>array(
					'name'=>'無効フラグ',
					'row_order'=>'Timetable.delete_flg',
					'clm_show'=>0,
			),
			'update_user'=>array(
					'name'=>'更新者',
					'row_order'=>'Timetable.update_user',
					'clm_show'=>0,
			),
			'ip_addr'=>array(
					'name'=>'IPアドレス',
					'row_order'=>'Timetable.ip_addr',
					'clm_show'=>0,
			),
			'created'=>array(
					'name'=>'生成日時',
					'row_order'=>'Timetable.created',
					'clm_show'=>0,
			),
			'modified'=>array(
					'name'=>'更新日',
					'row_order'=>'Timetable.modified',
					'clm_show'=>0,
			),
